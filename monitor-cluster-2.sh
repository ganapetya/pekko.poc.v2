...
while true; do
    ...
    HEALTH_RESPONSE=$(curl -s -w "%{http_code}" "http://localhost:8083/health" -o /tmp/monitor_health.txt 2>/dev/null)
    ...
    MEMBERS_RESPONSE=$(curl -s "http://localhost:8561/cluster/members" 2>/dev/null)
    ...
    SHARDS_RESPONSE=$(curl -s "http://localhost:8561/cluster/shards/CaseCompanion" 2>/dev/null)
    ...
    echo "   • Cluster members: curl -s http://localhost:8561/cluster/members | jq"
    echo "   • Shards: curl -s http://localhost:8561/cluster/shards/CaseCompanion | jq"
